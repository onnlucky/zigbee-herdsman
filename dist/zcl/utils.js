...
function getCluster(key) {
    ...
    if (typeof key === 'number') {
        for (const clusterName in definition_1.Cluster) {
            if (definition_1.Cluster[clusterName].ID === key) {
                name = clusterName;
                break;
            }
        }
...
function getSpecificCommand(clusterKey, direction, key) {
    const cluster = getCluster(clusterKey);
    const commands = direction === definition_1.Direction.CLIENT_TO_SERVER ? cluster.commands : cluster.commandsResponse;
    let name;
    if (typeof key === 'number') {
    ...
    if (!command) {
        throw new Error(`Cluster command with key '${key}' and direction '${definition_1.Direction[direction]}'` +
            `does not exist for cluster '${clusterKey}'`);
    }
    return command;
}
exports.getSpecificCommand = getSpecificCommand;
